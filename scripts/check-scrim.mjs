...
export const SCRIM = {
  ...
  // Из-за этого `bright` перестал быть пресетом и стал вторым умолчанием
  // плотности: покадровый scrimText перебивает его так же, как и общее
  // значение. Имя пережило смысл — переименование тронет content.js,
  // render.mjs и check-topbar-strip.mjs и вынесено в отдельную задачу.
  brightMax:   0.72,
  brightFade:  0.82,
