 ...
 *
 * Останавливаемся на главах. Сейчас глава есть у каждого кадра: один кадр —
 * одна остановка, и переход всегда ведёт к соседнему кадру. Механика кадров
 * без главы («проходных») сохранена и работает, но ни одного такого кадра в
 * стопке нет. Первая остановка отрицательная: там стоит первый экран,
 * лежащий отдельным слоем поверх стопки.
 */

/* ── масштаб ──────────────────────────────────────────────────────────
 * Кадр рождается уже во весь экран (1,040), на главе 1,150, к концу
 * растворения доходит до 2,60. softplus сглаживает излом: до главы кривая
 * почти плоская, после — резко уходит вперёд.
 *
 * Колено резче, чем было (accel 0,8 / soft 0,18). Опорные точки от этого не
 * сдвинулись: произведение accel · soft сохранено (0,145), а им и задаётся
 * масштаб на главе — 1,150 до сотой, как прежде. Изменилось, ГДЕ кадр
 * доходит до 2,6: раньше это приходилось на t = 1,14, а кадр растворяется
 * уже на 0,70 и до 2,6 не доживал — фактический потолок на экране был 1,83.
 * Теперь 2,6 приходится ровно на конец растворения: кадр дольше стоит на
 * крупности главы и уходит вперёд решительнее.
 */
 ...
 *  Кадров без главы в стопке нет, поэтому число сейчас ни на что не влияет:
 *  карта темпа выходит равномерной. Оставлено как часть механики. */
export const PASS_COST = 0.45;

/**
 * cubic-bezier(.33, 0, .18, 1) — та же кривая, что в токене --flight-ease.
 * Совпадение проверяет check-flight.mjs: по токену идут проявление секций и
 * подпись главы, по этой копии — сам пролёт, и разойтись они не должны.
 *
 * Кривая несимметричная нарочно. Прежняя (.65, 0, .35, 1) на длинном переходе
 * половину времени ползла у самых краёв пути — и трогалась вяло, и садилась
 * вяло. Здесь камера трогается раньше, а половина длительности уходит на
 * последнюю пятую часть пути, то есть на подъезд к главе. Пик скорости при
 * этом НЕ выше прежнего: 2,70 средней против 2,86 — рывка посередине не
 * прибавилось, изменилось распределение времени.
 */
function bezier(x) {
  const x1 = 0.33, x2 = 0.18;
  let u = clamp(x, 0, 1);
  for (let k = 0; k < 6; k++) {
    const mt = 1 - u;
 ...
 * Пока каждый кадр — глава, карта равномерна и движение задаёт одна кривая.
 */
