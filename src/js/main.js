 ...
 *
 *  Держится долей от перехода, а не абсолютным числом: 0,24 длительности —
 *  ровно та пропорция, что была при переходе 1000 мс. Оставить прежние 240 мс
 *  значило бы показать текст новой главы почти за две секунды до того, как
 *  камера на неё приедет: подпись уже стоит, а кадр под ней ещё едет. */
const CAPTION_SWAP = Math.round(DURATION * 0.24);   // 576 мс при 2400
/**
 * Насколько заранее просим кадры, которых ещё нет.
 *
 * Переход всегда ведёт к соседнему кадру, поэтому запас в два кадра покрывает
 * и текущую остановку, и следующую, и ту, что за ней. Пятёрка осталась от
 * четырнадцати кадров: при восьми она поднимала в простое всю стопку кроме
 * двух последних — то есть ленивая загрузка вырождалась в «загрузить всё»,
 * ради которой и разменивали LCP.
 *
 * Прогрев после `load` поднимает кадры 0–2; дальше каждое действие зовёт
 * onNeed и тянет ровно то, что скоро понадобится.
 */
const LOOKAHEAD = 2;
...
/* ------------------------------------------------------------------ *
 *  Кадры: разметка нулевого в HTML, остальные — в <noscript>
 * ------------------------------------------------------------------ */
